    ...
    <div className="min-h-screen bg-[#F8FAFC] text-slate-700 flex flex-col font-sans select-none">
      ...
      <main className="flex-1 p-6 md:p-8 max-w-5xl mx-auto w-full space-y-6">
        <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-xs space-y-6">
          ...
          {activeMetrics.cashDiscrepancy.hasIssue && (
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h4 className="text-xs font-bold text-rose-900 leading-tight">
                    Descuadre de Caja Detectado en Turno
                  </h4>
                  <p className="text-[11px] text-rose-700 mt-0.5">
                    Existe un faltante de{" "}
                    <strong>${Math.abs(activeMetrics.cashDiscrepancy.amount).toFixed(2)}</strong>{" "}
                    en el arqueo físico de efectivo.
                  </p>
                </div>
              </div>
              <Link
                href="/caja"
                className="flex items-center gap-1.5 px-3 py-1.5 bg-white border border-rose-200 hover:bg-rose-50 text-rose-700 text-xs font-bold rounded-lg shadow-2xs transition-colors"
              >
