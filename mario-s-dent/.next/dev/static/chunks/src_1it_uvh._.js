...
function CajaPage() {
    ...
    // Cálculo atómico y sanitizado de balances
    ...
    // Handlers con validación estricta previa a mutación
    ...
    const handleCloseShift = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useCallback"])({
        "CajaPage.useCallback[handleCloseShift]": async ()=>{
            if (totals.isShortage && !cashierNotes.trim()) {
                alert("Existe un faltante en el arqueo de efectivo. Debe registrar una justificación antes de emitir el Corte Z.");
                return;
            }
            const confirmed = window.confirm("¿Confirmas el cierre de jornada (Corte Z)? Esta acción registrará el balance final.");
            if (!confirmed) return;
            try {
                // TODO: Conectar endpoint POST /api/shifts/close
                closeShift();
                setCashierNotes("");
                setCountedCash(0.0);
                ...
                alert("No se pudo cerrar el turno. Intente nuevamente.");
            }
        }
    ...
    const handleResolveDiscrepancy = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useCallback"])({
        "CajaPage.useCallback[handleResolveDiscrepancy]": async ()=>{
            if (!adminNotes.trim()) {
                alert("Ingrese una justificación técnica contable para resolver la discrepancia.");
                return;
            }
            const confirmed = window.confirm("¿Aprobar el dictamen de esta auditoría y archivar la alerta?");
            if (!confirmed) return;
            try {
                // TODO: Conectar endpoint PATCH /api/shifts/:id/audit
                resolveAudit(adminNotes, resolutionType);
                alert("Discrepancia resuelta exitosamente.");
            } catch (err) {
                console.error("Error al auditar turno:", err);
                alert("No se pudo guardar la resolución.");
            }
        }
...
_s(CajaPage, "bbXqGrza/GgyWg4czab2GbLcthw=", false, function() {
    return [
...
//# sourceMappingURL=src_1it_uvh._.js.map